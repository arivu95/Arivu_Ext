...
class AppAuthWebPlugin extends FlutterAppAuthPlatform {
  ...
  Future<AuthorizationTokenResponse?> authorizeAndExchangeCode(AuthorizationTokenRequest request) async {
    ...
    return AuthorizationTokenResponse(tokenResponse.accessToken, tokenResponse.refreshToken, tokenResponse.accessTokenExpirationDateTime, tokenResponse.idToken,
        tokenResponse.tokenType, authResult.authorizationAdditionalParameters, tokenResponse.tokenAdditionalParameters);
  }
  ...
  static Future<AuthorizationTokenResponse?> processStartup(AuthorizationTokenRequest request) async {
  ...
  static AuthorizationResponse processLoginResult(String loginResult, String codeVerifier) {
  ...
  static Future<AuthorizationTokenResponse> exchangeCode(AuthorizationResponse authResult, String clientId, String redirectUrl, String tokenEndpoint,
      {String? clientSecret}) async {
    if (authResult.authorizationCode == null || authResult.authorizationCode!.isEmpty)
      throw ArgumentError(
          _AUTHORIZE_ERROR_MESSAGE_FORMAT.replaceAll("%1", _AUTHORIZE_AND_EXCHANGE_CODE_ERROR_CODE).replaceAll("%2", 'Login request returned no code'));
    ...
    if (response.statusCode != 200) {
      throw ArgumentError(_AUTHORIZE_ERROR_MESSAGE_FORMAT
          ...
          .replaceAll("%2", jsonResponse["error"]?.toString() ?? response.reasonPhrase ?? "Unknown Error"));
    }

    return AuthorizationTokenResponse(
        jsonResponse["access_token"].toString(),
        jsonResponse["refresh_token"] != null ? jsonResponse["refresh_token"].toString() : null,
        DateTime.now().add(new Duration(seconds: jsonResponse["expires_in"])),
        jsonResponse["id_token"].toString(),
        jsonResponse["token_type"].toString(),
  ...
  static Future<AuthorizationServiceConfiguration> getConfiguration(
    ...
    return AuthorizationServiceConfiguration(jsonResponse["authorization_endpoint"].toString(), jsonResponse["token_endpoint"].toString());
  }

  static Future<String> openPopUp(String url, String name, int width, int height, bool center, {String? additionalOptions}) async {
